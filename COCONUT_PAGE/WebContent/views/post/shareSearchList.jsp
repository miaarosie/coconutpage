                    ...
                    <div>
                        <form method="get" name="searchValue" action="<%=contextPath %>/shareSearch.share">
                                ...
                                <tr>
                                    <td>
                                        <select name="searchField" class="find_option form-control">
                                            ...
                                            <option value="postTitle" <%= searchField.equals("postTitle")? "selected" : ""%>>제목</option>
                                            <option value="empNo" <%= searchField.equals("empNo")? "selected" : ""%>>작성자</option>
                                        </select>
                                    </td>
                    
                                    <td>
                                        <input type="text" class="find_text form-control" placeholder="검색어를 입력해주세요" name="searchText" id="searchText" maxlength="50">
                                    </td>
                                    <td>
                         ...
                         <div align="center" class="pagination">
                   	     <% if(currentPage != 1) { %>
			             	<button onclick="location.href='<%=contextPath%>/shareSearch.share?currentPage=<%=currentPage-1%>&searchField=<%=searchField%>&searchText=<%=searchText%>'">&lt;</button>
			             <% } %>
			             
			             <% for(int p = startPage; p <= endPage; p ++) { %>
			             		<% if (p != currentPage) { %>
			             			
			             			<button onclick="location.href='<%= contextPath %>/shareSearch.share?currentPage=<%= p %>&searchField=<%=searchField%>&searchText=<%=searchText%>';"><%= p %></button>
