<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="com.kh.common.model.vo.PageInfo, java.util.ArrayList, com.kh.post.model.vo.Post2" %>
<%
	PageInfo pi = (PageInfo)request.getAttribute("pi");
	ArrayList<Post2> list = (ArrayList<Post2>)request.getAttribute("list");
		...
		<div class="container">
			...
			<div class="main">
				<div class="list">
					<% if(e != null) { %>
						<div id="list-btn">
							<a href="<%= contextPath %>/thingsEnrollForm.po" id="write_btn">글쓰기</a>
						</div>
					<% } %>	
					...
					<script>
						$(function() {
							$(".table>tbody>tr").click(function() {
								location.href="<%= contextPath %>/thingsDetail.po?pno="+ $(this).children().eq(0).val();
							});
						});	
					</script>
					...
					<div id="page">
						...
						<% if(currentPage != 1) { %>
							<button onclick="location.href='<%= contextPath %>/thingsList.po?currentPage=<%= currentPage-1 %>&searchword=<%=searchword %>&keyword=<%=keyword %>';">&lt;</button>
						<% } %>
							
						<% for(int pa = startPage; pa<= endPage; pa++) { %>
							<% if(pa != currentPage) { %>
								<button onclick="location.href='<%= contextPath %>/thingsList.po?currentPage=<%= pa %>&searchword=<%=searchword %>&keyword=<%=keyword %>';"><%= pa %></button>
							<% } else { %>
					...
					<div align="center">
						<form action="<%= contextPath %>/thingsList.po?currentPage=1" method="get" id="searchform">
							<table>
								<tr>
									<td>
